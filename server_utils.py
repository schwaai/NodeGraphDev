...
def update_json_dict(fname, data_dict=None, data_json=None):
    """Works with data_dict or data_json but not both!"""

    if data_dict is None and data_json is None:
        raise ValueError("data_dict or data_json must be provided")

    if data_dict is not None and data_json is not None:
    ...
    if data_dict is not None:
        ...
        json_data.update(json.loads(data_json))
