    ...
    for x in required_inputs:
        ...
        if isinstance(val, list):
            ...
            if r[val[1]] != type_input:
                return (
                    False, "Return type mismatch. {}, {}, {} != {}".format(class_type, x, r[val[1]], type_input),
                    unique_id)
...
def validate_prompt(prompt):
    ...
    for prompt_key in prompt:
        prompt_value = prompt[prompt_key]
        cls = prompt_value['class_type']
        class_ = nodes.NODE_CLASS_MAPPINGS[cls]
        if hasattr(class_, 'OUTPUT_NODE') and class_.OUTPUT_NODE == True:
            outputs.add(prompt_key)
