...
// Register a node for each definition
        for (const nodeId in defs) {
            ...
            // Create a new node using Object.assign to merge the node function and node properties
            const node = Object.assign(
                // Define the ComfyNode constructor function
                function ComfyNode() {
                    var inputs = nodeData["input"]["required"];

                    // If optional inputs exist, merge them with required inputs
                    ...
                    // Iterate over the inputs and configure them
                    for (const inputName in inputs) {
                        const inputData = inputs[inputName];
                        const type = inputData[0];

                        if (inputData[1]?.forceInput) {
                            // Add an input with the specified type
                            this.addInput(inputName, type);
                        } else {
                            if (Array.isArray(type)) {
                                // Enums: Use a combo widget if available
                                ...
                                // Node connection inputs: Add an input with the specified type
                                this.addInput(inputName, type);
                            }
                        }
                    }

                    // Configure the outputs of the node
                    for (const o in nodeData["output"]) {
                        const output = nodeData["output"][o];
                        const outputName = nodeData["output_name"][o] || output;
                        // Add an output with the specified name and type
                        this.addOutput(outputName, output);
                    }

                    // Compute the size of the node and update the size property
                    ...
                    // Invoke the "nodeCreated" extension asynchronously
            ...
            // Further operations can be performed with the created node
